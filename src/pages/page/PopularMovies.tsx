...
export default function PopularMovies() {
  ...
  useEffect(() => {
    fetchPopularMovies()
      .then((popularMovies) => {
        ...
        setDefaultMovies(popularMovies);
      })
      .catch((error) => {
        console.error(error);
      });
  ...
  return (
    <div className="bg-cover min-h-screen background">
      <Navbar />
      {display && <DetailMovies setDisplay={setDisplay} id={id} data={popularMovies} />}
      <h1 className="flex justify-center text-4xl text-white pt-4 pb-4">Popular Movies</h1>
